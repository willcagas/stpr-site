...
void main() {
  ...
  // Define colors from uniforms
  ColorStop colors[3];
  colors[0] = ColorStop(uColorStops[0], 0.0);
  colors[1] = ColorStop(uColorStops[1], 0.5);
  colors[2] = ColorStop(uColorStops[2], 1.0);
  
  // Color Ramp Logic inline
  vec3 rampColor;
  int index = 0;
  for (int i = 0; i < 2; i++) {
...
const Aurora: React.FC<AuroraProps> = (props) => {
  // CHANGED: Default to Deep Mystic Colors: Indigo -> Gold -> Violet
  const { colorStops = ['#4f46e5', '#fbbf24', '#7c3aed'], amplitude = 1.0, blend = 0.8 } = props;
  ...
  // CHANGED: Use mix-blend-screen for glowing light effect on dark background
  return <div ref={ctnDom} className="w-full h-full absolute inset-0 z-0 mix-blend-screen opacity-60" />;
}
